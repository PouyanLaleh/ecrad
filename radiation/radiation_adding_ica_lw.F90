...
contains
  ...
  ! atmospheric layers.
  subroutine adding_ica_lw(ng, nlev, &
  ...
  subroutine fast_adding_ica_lw(ng, nlev, &
       &  reflectance, transmittance, source_up, source_dn, emission_surf, albedo_surf, &
    ...
    do jg = 1,ng
      do jcol = istartcol,iendcol
        if (F_div(jcol)) then
          flux_dn(jcol,jg,1:i_cloud_top(jcol)) = flux_dn_clear(jcol,jg,1:i_cloud_top(jcol))

          albedo(jcol,jg,nlev+1) = albedo_surf(jcol,jg)
    
        ! At the surface, the source is thermal emission
          source(jcol,jg,nlev+1) = emission_surf(jcol,jg)
    ...
    do jlev = nlev,MINVAL(i_cloud_top(istartcol:iendcol)), -1
      do jg = 1, ng
        do jcol = istartcol, iendcol
          IF ( i_cloud_top(jcol) > jlev ) CYCLE
            if (F_div(jcol)) then
      ...
      do jg = 1, ng
          do jcol = istartcol,iendcol
            ...
            ! Compute the fluxes above the highest cloud
              flux_up(jcol,jg,i_cloud_top(jcol)) = source(jcol,jg,i_cloud_top(jcol)) &
             &                 + albedo(jcol,jg,i_cloud_top(jcol))*flux_dn(jcol,jg,i_cloud_top(jcol))

            end if
          end do
      end do

    do jlev = MAXVAL(i_cloud_top(istartcol:iendcol))-1, 1, -1
      do jg = 1, ng
        do jcol = istartcol, iendcol
          IF ( i_cloud_top(jcol)-1 < jlev ) CYCLE
            if (F_div(jcol)) then
              if (is_clear_sky_layer(jcol,jlev)) then
   ...
   do jlev = MINVAL(i_cloud_top(istartcol:iendcol)), nlev
      do jg = 1, ng
        do jcol = istartcol, iendcol
          IF ( i_cloud_top(jcol) > jlev ) CYCLE
            if (F_div(jcol)) then
  ...
  subroutine calc_fluxes_no_scattering_lw(ng, nlev, &
       &  transmittance, source_up, source_dn, emission_surf, albedo_surf, flux_up, flux_dn, istartcol, iendcol)
